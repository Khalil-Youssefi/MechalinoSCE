	...
	void handle_command(void)
	{
		if (command != 'X')
		{
			...
			{
				Motors_SetPWM(&motors, FWD_L, FWD_R);
			}
			else if (command == 'B')
			{
				Motors_SetPWM(&motors, FWD_R, FWD_L);
			}
			else if (command == 'R')
			{
				Motors_SetPWM(&motors, FWD_L, FWD_L);
			}
			else if (command == 'L')
			{
				Motors_SetPWM(&motors, FWD_R, FWD_R);
			}
			else if (command == 'W')
			{
				Motors_SetPWM(&motors, FWD_L, FWD_R);
			...
			if (command == 'F' || command == 'B'|| command == 'R' || command == 'L')
			{
				...
				tick = HAL_GetTick();
				if (params[0] != 0.0f)
					duration = (uint32_t)(params[0] * 1000.0f);
				else
					duration = 3000;
			}
		}
	...
	int main(void)
	{
		...
		while (1)
		{
			...
			uint32_t diff = tock - tick;

			if (RW_mode == 0)
			{
				if (diff >= duration && duration > 0){
					duration = 0;
					Motors_Stop(&motors);
				}
				if (adc_readings[0] > OBSTACLE_TH0_MV || adc_readings[1] > OBSTACLE_TH1_MV || adc_readings[2] > OBSTACLE_TH1_MV)
				{
					duration = 0;
					Motors_Stop(&motors);
				}
				...
				if (adc_readings[0] > OBSTACLE_TH0_MV || adc_readings[1] > OBSTACLE_TH1_MV || adc_readings[2] > OBSTACLE_TH2_MV)
				{
					Motors_SetPWM(&motors, FWD_L, FWD_L);
					HAL_Delay(2000);
				}
